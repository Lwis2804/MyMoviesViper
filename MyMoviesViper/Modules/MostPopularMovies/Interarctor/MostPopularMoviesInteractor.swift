...
class MostPopularMoviesInteractor: MostPopularMovies_PresenterToInteractorProtocol {
    ...
    func getMostPopularMoviesToInteractor() {
        print("\n\n\n\n on interactor   ---->\n\n\n\n")
        let service : NetworkApiProtocol = MoviesWebService(urlConfiguration: MostPopMovUrlConfig(metodo: metodo, host: host, path: mostPopularMoviesPath.getMostPopularMovies.getPath()))
